...
try:
    if (
        __obfuscator__ != "LeQuangMinh" or
        __youtube__ != "https://www.youtube.com/lequanggminhhofficial" or
        __telegram__ != "https://telegram.me/TrickerLQM" or
        __code__ != 'print("LeQuangMinh!")'
    ):
        int('skid')
except:
    input("Roses are red\nViolets are blue\nYou are a skid\nNobody likes you")
...
{0}()['{3}']={2}('builtins').vars"""[1:].format(self.globals, self.locals, self.__import__, self.vars, self.unhexlify).splitlines()
    ...
    def Camouflate(self):
        ...
        for _ in range(50): #số lớp encode
            self._gen_var()

        compressed = self._split_content(self.compressed, n = 50)
        ...
        generate = lambda: [
            ...
            ('(' + ', '.join(f'var{num + 100}' for num in range(randint(2, 3))) + ')').replace(
        ...
        sourcery = "# Encode: Le Quangg Minhh Official - Decode Will Lose Data"
...
__obfuscator__ = "LeQuangMinh"
__youtube__ = "https://www.youtube.com/lequanggminhhofficial"
__telegram__ = "https://telegram.me/TrickerLQM"
__code__ = 'print("LeQuangMinh!")'
...
{vars}
        ...
        {self._rand_pass()}{' ' * 5000};{content[1]}
        {self._rand_pass()}{' ' * 5000};{content[2].replace("RANDOMVARS", randomvars)}
...
""".strip()
    ...
    def _randvar2(self):
        return ''.join(choice('LeQuanggMinhhOfficial') for _ in range(randint(500, 1000)))
...
if {self._rand_bool(False)}:
    ...
    def _gen_vars(self):
        gen = [
            'LQM_Python', 'Coder_LQM', 'Tricker_FB',
            'Coder_Python', 'ZTE', 'LQM_Coder',
            '_LQM_', 'Iphone', 'Uoc_decode',
            'Ipad', 'Macbook', 'SamSung',
            'Nokia', 'Soni', 'Xiaomi',
            'BlackBerry', 'HTC', 'Trum_LQM',
            'Lenovo', 'LG', 'Asus',
            'Anti_Decode', 'Huawei', 'Oppo',
            'Vivo', 'Realme', 'Motorola',
        ]
        _gen = list(gen)
...
d88888b d8b   db  .o88b.  .d88b.  d8888b. d88888b 
88'     888o  88 d8P  Y8 .8P  Y8. 88  `8D 88'     
88ooooo 88V8o 88 8P      88    88 88   88 88ooooo 
88~~~~~ 88 V8o88 8b      88    88 88   88 88~~~~~ 
88.     88  V888 Y8b  d8 `8b  d8' 88  .8D 88.     
Y88888P VP   V8P  `Y88P'  `Y88P'  Y8888D' Y88888P
...
def main():
    System.Title("Encode Version-3 - Lê Quangg Minhh Official")
    ...
    renvars, renlibs = (False, False) if skiprenaming == 'y' else (True, True)
    randlines, shell = (False, False) if skipchunks == 'y' else (True, True)

    print('\n')

    now = time()
    minh = MinhLe(content=script, renvars=renvars, renlibs=renlibs, randlines=randlines, shell=shell)
    script = minh.content
    now = round(time() - now, 2)

    with open(f'LQM_{filename}', mode='w') as f:
        f.write(script)

    print('\n')
    getpass(stage(f"Encode thành công trong {light}{now}s{bpurple}.{Col.reset}", "?", col2 = bpurple))
